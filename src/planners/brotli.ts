 ...
 * reads the patterns, and only when a file turns out to need it, so a feed published as gzip — or
 * a browser that grows brotli — never pays for it.
 */
...
/** How much to ask the decoder for at a time. Big enough that a 33MB file is not a million calls. */
 ...
 *
 * Everything else is left alone: gzip announces itself, and a plain pattern file starts with the
 * front-coding count of its first line, which is an ASCII digit. Only what is neither is treated as
 * brotli, which is what the file is written with.
 */
...
/** Decompress a whole brotli file. For the per-station files, which are tens of kilobytes. */
export async function brotliDecompress(bytes: Uint8Array): Promise<Uint8Array> {
  await wasm();
  return brotli_dec(bytes);
}
 ...
 * can be many out, since a megabyte of patterns compresses hard.
 */
...
/**
 * The bytes of a pattern file, decompressed if it is brotli and untouched if it is not.
 *
